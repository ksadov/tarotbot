...
def draw5img(cards, cardwidth: int, cardheight: int) -> Image.Image:
    ...
    img.paste(cards[1], (2 * image_border + cardwidth, 2 * image_border + cardheight))
    img.paste(
        cards[2], (2 * image_border + cardwidth, 3 * image_border + 2 * cardheight)
    )
    img.paste(cards[3], (image_border, 2 * image_border + cardheight))
    img.paste(
        cards[4], (3 * image_border + 2 * cardwidth, 2 * image_border + cardheight)
    )
    return img
...
def genericimg(cards, cardwidth: int, cardheight: int) -> Image.Image:
    ...
    for i in range(0, rows):
        offset = (
            round((total_width - lastrow_width - image_border) / 2)
            if i == rows - 1
            else 0
        )
        for j in range(0, cols):
            ...
            img.paste(
                cards[i * cols + j],
                (
                    j * (image_border + cardwidth) + offset,
                    i * (image_border + cardheight),
                ),
            )
    return img
