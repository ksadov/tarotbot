...
class Card:
    ...
    def __init__(self, name: str, upright: str, reverse: str, code: str, up=True):
        self.name = name
...
for deck in listdir(path.join("decks", "tarot")):
    with open(path.join(deck, "deck.json"), "r") as f:
        deck_data = json.load(f)
        new_cards = list(
            map(
                lambda card_data: Card(
                    card_data["name"],
                    card_data["upright"],
                    card_data["reverse"],
                    card_data["code"],
...
def makeImgList(cards: List[Card], deck: Decks):
    ...
    for c in cards:
        newcard = Image.open(
            path.join(
                path.dirname(__file__), "..", "decks", deck.shortname, c.code + ".jpg"
            )
        ).convert("RGBA")
        if not c.up:
